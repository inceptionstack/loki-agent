...
_telem_machine_id() {
  ...
  raw="${raw}:$(hostname 2>/dev/null || echo unknown)"
  if command -v sha256sum &>/dev/null; then
    echo "sha256:$(echo -n "$raw" | sha256sum | cut -d' ' -f1)"
  elif command -v shasum &>/dev/null; then
    echo "sha256:$(echo -n "$raw" | shasum -a 256 | cut -d' ' -f1)"
  else
    # Fallback: use the install_id (less stable but non-identifying)
    echo "fallback:${_TELEM_INSTALL_ID}"
  fi
}
...
_telem_send_install_beacon() {
  ...
  arch_name="$(hw_arch 2>/dev/null || uname -m 2>/dev/null)"
  os_ver="$(uname -r 2>/dev/null | cut -d. -f1-2)"
...
EOF
  ...
  # Synchronous here (at exit) but with hard timeout — won't block >2s
  curl -sfSL -X POST \
    "${_TELEM_ENDPOINT}/v1/ingest" \
    -H "Content-Type: application/json" \
    --connect-timeout "$_TELEM_TIMEOUT" \
    --max-time "$_TELEM_TIMEOUT" \
    -d "$body" \
