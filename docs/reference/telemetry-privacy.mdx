...
Telemetry is sent from `install.sh` (and its helper `lib/telemetry.sh`) to `https://telemetry.loki.run`. Three kinds of calls may happen:
...
- [`lib/telemetry.sh`](https://github.com/inceptionstack/lowkey/blob/main/lib/telemetry.sh) — the complete telemetry client (≈320 lines of bash)
